...
contract Multisig {
    ...
    // signer -> quorumUpdateId/ -> bool (checking if an address has signed)
    ...
    function updateQuorum(uint8 _quorum) external {
        require(isValidSigner[msg.sender], "not a valid signer"); // make sure that only one of the valid signers can update the quorum
